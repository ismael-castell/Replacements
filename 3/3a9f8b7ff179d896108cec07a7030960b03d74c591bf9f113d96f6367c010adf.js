...
* License: MIT
*
* JIz: modified _applyDataApi method to produce groups in correct order
*/
...
;(function(factory) { // eslint-disable-line no-extra-semi
    ...
    */
    ...
    matchHeight._applyDataApi = function() {
        var indexMap = {},
          groups = [],
          $elements = $('[data-match-height], [data-mh]');

        // generate groups by their groupId set by elements using data-match-height
        // JIz: modified so that groups are sorted in the document order according to the first member
        $elements.each(function() {
            var $this = $(this),
                groupId = $this.attr('data-mh') || $this.attr('data-match-height'),
                index = indexMap[groupId];

            if ('undefined' == typeof index) {
              index = groups.length;
              indexMap[groupId] = index;
              groups[index] = $this;
            } else {
              groups[index] = groups[index].add($this);
            }
        });

        // JIz: order is then modified so that ancestors follow their descendants
        // (only approximation, yet valid for "sane" usage)
        var i = 0;
...
(function() {
	...
	var popUp = function($elm) {
		if ($elm && $elm.length) {
			this.$elm = $elm;
			this.$controller = $elm.data("toggle");
			this.init();
		}
	};
	...
	$.extend(popUp.prototype, {
		init: function () {
			var $elm = this.$elm;
			if (!document.cookie.split(';').filter(function(){}