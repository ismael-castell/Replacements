    ...
    function Tabs(config) {
        ...
        function bindEvents() {
            ...
            if (tabs && Array.isArray(tabs)) {
                const tabsParent = tabs[0].parentNode;
                let orientationFlag = false;
                if(tabsParent.hasAttribute('aria-orientation')) {
                    orientationFlag = tabsParent.getAttribute('aria-orientation') === 'horizontal' ? false : true;
                } 
                for (var i = 0; i < tabs.length; i++) {
                    (function(index) {
                        ...
                        tabs[i].addEventListener("keydown", function(event) {
                            onKeyDown(event, orientationFlag);
                        });
                    })(i);
                }
            }
         ...
         * @param {Object} event The keydown event
         */
        function onKeyDown(event, orientationFlag) {
            var index = that._active;
            var lastIndex = that._elements["tab"].length - 1;
            if(!orientationFlag) { // horizontal
                switch (event.keyCode) {
                    case keyCodes.ARROW_LEFT:
                        event.preventDefault();
                        if (index > 0) {
