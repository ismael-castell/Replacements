...
var livePlayerVer = "2022.04.24.02";
var livePlayerObjs = {};
...
function createLivePlayer(paras) {
    ...
    /*兼容回看返回直播时，页面没有吧playerType设置为live问题*/
    if(isIPad() && paras.playerType==="liveback" && paras.start) {
        paras.playerType = "live";
        paras.start = "";
        paras.end = "";
    }
    ...
    //220204,提示信息
    if(isIPad() && showNoDrmMsg(paras) && !isIosDrmPlayer(paras)) {
        return;
    }
    ...
    if(isIPad() || isLiveHlsJsSupported() && isWasmSupported() && navigator.userAgent.indexOf("rv:11")===-1 && navigator.userAgent.indexOf("MSIE")===-1) {
        ...
        if(isTimeshift(paras) && typeof createLiveHls !== "undefined" && !paras.vdnRetryNum) {
        ...
        var workerUrl = "http://js.player.cntv.cn/creator/h5.worker?v=211215";
        ...
        if(paras.isHttps === "true") {
            ...
            workerUrl = "https://js.player.cntv.cn/creator/h5.worker?v=211215";
        }
    ...
    */
        ...
        if((navigator.userAgent.indexOf("rv:11")===-1 && navigator.userAgent.indexOf("MSIE")===-1) || paras.vdn && paras.vdn.vtoken || (paras.others==="flv"||typeof paras.others==="string" && paras.others.indexOf(".flv")>0) || location.href.indexOf("olympicchannelchina.cn")!==-1 || location.href.indexOf("ipanda.cn")!==-1 || location.href.indexOf("csmpte.com")!==-1 || (paras.playerType==="live_homepage"||paras.playerType==="small")&&location.href.indexOf("cctv.cn")!==-1) {
            var showMsg = "<p>您当前的浏览器不支持付费视频播放</p>" + "<p>请更换浏览器或设备</p>" + "<p>(如果是360、QQ或搜狗浏览器，须使用极速模式观看)</p>";
...
function isNewH5LivePlayer(paras) {
    ...
    /*
   var h5TvChannels = "zsslsfwsgz,cctv1,cctv2,cctv3,cctv4,cctveurope,cctvamerica,cctv5,cctv5plus,cctv6,cctv7,cctv8,cctvjilu,cctv10,cctv11,cctv12,cctv13,cctvchild,cctv15,cctv17,ipanda,";
   var h5DomainUrl = location.href.toLowerCase().indexOf("tv.cctv.com/live")!==-1 || location.href.toLowerCase().indexOf("wangchun.cctv.com")!==-1 || location.href.toLowerCase().indexOf("chunwan.cctv.com")!==-1 || location.href.toLowerCase().indexOf("wlchunwan.cctv.com")!==-1;


    var h5DomainUrl = location.href.toLowerCase().indexOf("cctv.com")!==-1 || location.href.toLowerCase().indexOf("cntv.cn")!==-1 || location.href.toLowerCase().indexOf("gov.cn")!==-1 || location.href.toLowerCase().indexOf("12371.cn")!==-1 || location.href.toLowerCase().indexOf("ncpa-classic.com")!==-1;

    if(!h5DomainUrl) {
        h5DomainUrl = location.href.toLowerCase().indexOf("ipanda.com")!==-1 || location.href.toLowerCase().indexOf("chncpa.org")!==-1 || location.href.toLowerCase().indexOf("docuchina.cn")!==-1;
    }
     */
...
function getTimescopeCopyrightData(paras, timeScopeDate) {
    timeScopeUrl = "http://cbox.cntv.cn/epg/ctlist/" + timeScopeDate + "/" + paras.t + ".json";
    if(paras.isHttps === "true") {
    ...
    if(data && !data.divId) {
        ...
        for(var i=0; i<len; i++){
            ...
            if(data[i].url) {
                ...
                livePlayerObjs.adBannerAPI[i].clickUrl = data[i].clickUrl;
...
function initLiveH5Player(paras) {
    ...
    if(paras.playerType === "hw") {
        ...
        if(errorMsgDiv && errorMsgDiv.style.display!=="none" && errorMsgDiv.innerText.indexOf("听")===-1) {
            return;
        }
    }
    ...
    if(livePlayerObjs[paras.divId].adCalls && livePlayerObjs[paras.divId].adCalls.length>3 && livePlayerObjs[paras.divId].vdn.public==="1") {
        ...
        if(!isIPad() && paras.hasBarrage === "true" && !isLiveBarrageJsLoaded && paras.barrageitemID && paras.barrageApp) {
            ...
            barrageJs = "html5player_barrage.js"
            ...
            LazyLoad.js(barrageJs, function(){
                ...
                if(typeof Html5PlayerBarrage !== "undefined") {
                    ...
                    var barrs = {
                        ...
                        speed: 1 //弹幕倍速，以默认若干倍的速度，比如2,0.5等
                    };
...
function parseLiveDataFromVdnWhenError(paras, errStr) {
    ...
    if(typeof livePlayerObjs[paras.divId].video.url !=="string" || livePlayerObjs[paras.divId].video.url.length<10) {
        ...
        if(!errStr) {
            ...
            setLiveWsMetadata(paras, "VDN_REQUEST_FAILED");

            setCntvLiveMetadata(paras, "690003", "VDN_REQUEST_FAILED");
...
function parseLiveDataFromVdn(paras, isDirectVideoUrl, vData) {
    ...
    var videoUrl = "";
    var errStr = "";
    var videoData = vData ? vData : html5VideoData;
    if(livePlayerObjs[paras.divId].vdnTimer !== undefined) {
        clearTimeout(livePlayerObjs[paras.divId].vdnTimer);
    }
    ...
    if(!isDirectVideoUrl) {
        ...
        try{
            obj = eval('(' + videoData + ')');
            if(obj.ack.toLowerCase()!=="yes" || obj.status==="0") {
                ...
                if(isLiveSdrmPlayer(paras)) {
                    //videoUrl = "http://39.96.9.37/cdrmlive/test_1/index.m3u8";
                    //videoUrl = "https://cctvtxyh5ca.liveplay.myqcloud.com/live/cctv13_2/index.m3u8?contentid=2820180516001"
                ...
                if(livePlayerObjs[paras.divId].streamType==="flv") {
                    ...
                    if(typeof paras.others==="string" && paras.others.indexOf(".flv")>0) {
                        ...
                        livePlayerObjs[paras.divId].flvList = [{bitrate:bitRate, url:paras.others}];
                        livePlayerObjs[paras.divId].vdn.flvCdnName = "unknown";
                    } else{
                        obj.flv_url.flv1 = "https://cctv5alih5flvc.v.myalicdn.com/live/cdrmcctv5_1pd.flv";
                        obj.flv_url.flv2 = "https://cctv5alih5flvc.v.myalicdn.com/live/cdrmcctv5_1pd.flv";
                        obj.flv_url.flv3 = "https://cctv5alih5flvc.v.myalicdn.com/live/cdrmcctv5_1pd.flv";
                        obj.flv_url.flv4 = "https://cctv5alih5flvc.v.myalicdn.com/live/cdrmcctv5_1pd.flv";
                        if(obj.flv_url && obj.flv_url.flv1 && obj.flv_url.flv1.indexOf(".flv")>0) {
                            livePlayerObjs[paras.divId].flvList.unshift({bitrate:3600000, url:obj.flv_url.flv1});
                        }

                        if(obj.flv_url && obj.flv_url.flv2 && obj.flv_url.flv2.indexOf(".flv")>0) {
                            livePlayerObjs[paras.divId].flvList.unshift({bitrate:1800000, url:obj.flv_url.flv2});
                        }

                        if(obj.flv_url && obj.flv_url.flv3 && obj.flv_url.flv3.indexOf(".flv")>0) {
                            livePlayerObjs[paras.divId].flvList.unshift({bitrate:1350000, url:obj.flv_url.flv3});
                        }
                        if(obj.flv_url && obj.flv_url.flv4 && obj.flv_url.flv4.indexOf(".flv")>0) {
                            livePlayerObjs[paras.divId].flvList.unshift({bitrate:900000, url:obj.flv_url.flv4});
                        }
                ...
                if(obj.public === "0" && !isTimeshift(paras) && paras.playerType!=="liveback") {
        ...
        var errObj = null;
        if(errStr!=="retry" && typeof obj === "object" && obj && paras.playerType!=="liveback") {
            errObj = liveCheckVdnReturnMsg(obj);
    ...
    */
    ...
    window.addEventListener('blur', function(){
        ...
        if((navigator.userAgent.toLowerCase().indexOf("micromessenger")>0||navigator.userAgent.toLowerCase().indexOf("huaweibrowser")>0||navigator.userAgent.toLowerCase().indexOf("weibo")>0) && obj) {
            ...
            if(obj.getAttribute("isPageFullscreen") === "true") {
                ...
                if(player) {
                    ...
                    if(isCanvasSupported(divId)) {
                        ...
                        if(pageFullBtn && pageFullBtn.getAttribute("isPageFullscreen")==="true") {
                            ...
                            LiveMobileFullscreenBtn.pageFullscreenClick(divId, "yes");
                        }
                        */
                        clearInterval(livePlayerObjs[divId].canvasDrawTimer);
...
function createLiveConvivaSession(paras, errorMsg) {
    if(liveConvivaClient && liveConvivaPlayerStateManager) {
        ...
        if(errorMsg === "VDN_REQUEST_FAILED") {
        ...
        var errorMsgDiv = document.getElementById("error_msg_"+paras.divId);
        if(livePlayerObjs[paras.divId].video&&livePlayerObjs[paras.divId].video.audioUrl&&errorMsgDiv&&errorMsgDiv.innerText.indexOf("听")>0) {
...
function setCntvLiveMetadata(paras, msgType, errorMsg) {
    ...
    if(typeof(goldlog) != "undefined" && typeof(goldlog.startSession) != "undefined") {
        if(msgType == "init") {
            ...
            goldlog.record("/play.1.4","","&type=H5&playtime=0&loadtime=0" + para,"");
        } else{
        ...
        if(errorMsg.indexOf("听")===-1) {
            ...
            if(fullObj && fullObj.getAttribute("isPageFullscreen") === "true") {
                ...
                if(container.contains && container.contains(player)) {
                    ...
                    if(/(iphone|ipad)/i.test(navigator.userAgent) && document.exitPictureInPicture) {
